...
- `docs/guides/investigate-your-pipeline.md`, `docs/guides/evidence-limitations.md`, `docs/guides/migrating-to-focused-retobs.md` — the Investigate walkthrough on the demo, evidence limits and audit semantics, and the 0.7.0 migration (APIs, v2→v3 policy conversion, links and redirects, 410 routes, storage migrate/index, removed extras, pinned 0.6.0 / 29c67b8 reproduction).
...
- `README.md` and `docs/*` rewritten around connect → investigate → audit with verified commands and the measured support boundary; `tracelens.md` and `counterfactual-replay.md` reduced to retirement notes; `candidate-lineage-explorer.md` points to Investigate; the hosted demo is marked pending redeploy until its databases carry investigation tables.
- `scripts/generate_demo_assets.py` — captures Investigate (lost document, comparison) and Audit on `retobs demo` output, refuses pages showing local paths, manifest schema 2; `docs/assets/` regenerated (`investigate-lost-document.png`, `investigate-compare.png`, `audit-pass.png`, `retrieval-debugging-loop.webm`); `scripts/check_markdown_links.py` also checks `RETOBS_MASTER_PLAN.md` when present and requires the new guides.
- `release/audit.py`, `dashboard/api.py`, `cli.py`, `sdk/report.py` — changed queries are taken from the policy's decision metric (the recovered demo query now appears), and every emitted dashboard link is a focused route (`#/investigate?…&compare=`, `#/audit?baseline=&candidate=`) instead of the retired `#/runs`/`#/compare` pages; the UI's guide links point at GitHub.
- `tests/unit/test_comparison_delta_orientation.py` — the demo test reads `demo_manifest.json` and asserts the validation run recovers `q-outage`.
...
- `store/base.py`, `store/sqlite.py`, `store/postgres.py`, `evidence/service.py` — an unreadable stored trace raises `TraceDecodeError` naming the trace (never its payload) and investigation routes answer 422 `trace_unreadable` instead of an uncoded 500.
- `tracing/candidates.py` — candidate transitions read fields once per item and rank outputs through first-occurrence lookups instead of an inputs×outputs scan (capture overhead on an 8-operator, 100-candidate pipeline: 11.9 ms → 4.8 ms per call); captured data unchanged.
- `runner/execute.py` — `evaluate(..., chunk_map=)` / `retobs evaluate --chunk-map` with gold labels scores chunk results against the caller's document-level qrels (as `judgments=JudgmentSet.from_qrels(qrels)`); previously chunk ids were scored against document ids and recall was 0. The dataset fingerprint keeps the caller's qrels so chunk-mapped and document-level runs stay comparable. Each judged document takes the namespace its `chunk_map` rows give it (`default` otherwise); a document mapped under two namespaces raises `ValueError`. `datasets/judgments.py::ChunkMap.chunk_ref` resolves a candidate that carries no namespace to the only namespace its chunk id is mapped under (explicit namespace first, then `default`); scoring and the investigation projection share it, and an id mapped under several namespaces stays unmapped instead of being guessed.
- `evidence/service.py` — a projection whose store write fails (read-only file, disk I/O error, full disk) returns 409 `projection_write_failed` instead of an uncoded 500; the previous projection is left intact.
- `sdk/observe.py` — span `params` no longer copy candidate-list arguments (`candidates`, `hits`, `chunks`, … and arguments named after declared parents), so captured document text no longer leaks into the recorded configuration; arguments whose values a `CaptureSpec` reads as candidate input are omitted too.
- `tracing/sink.py` — `FlushResult.failed` counts traces whose export permanently failed while that flush waited; a flush after a failed export no longer reads as clean (`unflushed=0`) with nothing persisted.
- `dashboard/ui` — Investigate graph cards reserve room for the selected journey's path line (labels no longer overlap on highlighted nodes) and the selected card says "selected"; an index build or "load more" response for a scope that is no longer selected is dropped and a cursor is never requested twice; the operator graph is exposed as a group of buttons to assistive technology; investigation and audit tables scroll inside their own container at 390 px instead of collapsing columns. Scroll regions (operator, audit, Connect tables and command blocks) are keyboard-focusable labelled regions and inline links are underlined (axe WCAG 2 A/AA `scrollable-region-focusable`, `link-in-text-block`).
...
- `docs/assets/{comparison,query-debugger,validated-fix}.png` (replaced by the regenerated demo captures).
