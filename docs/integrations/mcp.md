...
Use `integrate_project` for the reviewed plan/apply/verify sequence. It accepts `project_root`, `phase`, an optional `framework` override, and for apply the reviewed `plan` or `plan_path` of the saved plan. Verify reads the applied `retobs/integration.yaml`; a `plan`/`plan_path` given to verify must match its `plan_id`. A relative `db_path` resolves against `project_root`. See [AGENT_QUICKSTART.md](AGENT_QUICKSTART.md).
