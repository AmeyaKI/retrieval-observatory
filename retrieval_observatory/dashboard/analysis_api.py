...
def build_analysis_router(store_for: Callable[[str], Any]) -> APIRouter:
    ...
    async def inputs(
        ...
        return store, traces, AnalysisScope(db_id, service_id, run_id, since, until)
    ...
    async def gates(
        ...
        store, traces, scope = await inputs(db_id, service_id, run_id, since, until)
    ...
    async def latency(
        ...
        _, traces, scope = await inputs(db_id, service_id, run_id, since, until)
        return analyze_latency(traces, scope).to_dict()

    @router.get("/ground-truth")
    async def ground_truth(db_id: str, run_id: str = Query(...)):
        store, _, scope = await inputs(db_id, None, run_id)
    ...
    async def instrumentation(
        ...
        store, traces, scope = await inputs(db_id, service_id, None, since, until)
