  ...
  return (
    <div className="space-y-6">
      <section aria-labelledby="query-evidence-heading" className="rounded-xl border border-slate-300 dark:border-slate-700 bg-surface p-4">
        <h2 id="query-evidence-heading" className="text-base font-semibold text-ink">
          {evidence.query.text ?? 'Query text unavailable'}
        </h2>
        <dl className="mt-3 grid gap-3 text-xs sm:grid-cols-3">
          ...
          <div><dt className="text-ink-faint">Ground truth</dt><dd className="text-ink">{evidence.ground_truth.evidence_class} · {evidence.ground_truth.relevant_doc_ids.length} relevant chunk(s)</dd></div>
        </dl>
        {evidence.ground_truth.relevant_doc_ids.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1" aria-label="Relevant documents">
            {evidence.ground_truth.relevant_doc_ids.map((docId) => (
              <span key={docId} className="rounded border px-1.5 py-0.5 font-mono text-[11px]">
                {docId} (grade {evidence.ground_truth.grades[docId]})
              </span>
            ))}
          </div>
        )}
      ...
      <CandidateFlowWorkspace dbId={dbId} runId={runId} queryId={queryId} />
      ...
      <details className="rounded-lg border border-slate-200 dark:border-slate-700 p-3">
        <summary className="cursor-pointer text-sm font-medium text-ink-muted">
          Advanced: origin, operator waterfall, regressions
        </summary>
          ...
          {queryTraces.length === 0 ? (
            ...
            <div>
              ...
              <p className="text-xs text-ink-muted mb-3">
                {queryTraces.length} pipeline trace{queryTraces.length === 1 ? '' : 's'}. Prefer the flowchart above for miss diagnosis; use this waterfall for raw operator diffs.
              </p>
