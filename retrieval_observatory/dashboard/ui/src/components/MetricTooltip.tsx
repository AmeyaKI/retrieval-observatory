...
export function MetricTooltip({ text, alignLeft = false }: Props) {
  ...
  return (
    <span ref={ref} className="relative inline-block align-middle ml-1">
      <button
        className="w-4 h-4 rounded-full bg-gray-200 hover:bg-gray-300 text-gray-500 hover:text-gray-700 text-[10px] font-bold leading-none flex items-center justify-center transition-colors"
        onClick={() => setOpen((v) => !v)}
      ...
      {open && (
        <div
          className={`absolute z-50 w-72 bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-xs text-gray-600 leading-relaxed ${
