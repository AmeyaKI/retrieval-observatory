    ...
    <div className="space-y-1.5">
      {entries.map(([k, n]) => (
        <div key={k} className="flex items-center gap-2 text-xs">
          <span className="w-28 shrink-0 text-gray-600 dark:text-slate-300 truncate" title={k}>{k}</span>
          ...
          <span className="w-16 text-right tabular-nums text-gray-500 dark:text-slate-400">{n} ({Math.round((n / total) * 100)}%)</span>
        </div>
      ))}
    </div>
  )
...
function Card({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) {
  return (
    <div className="rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4">
      ...
      {subtitle && <p className="text-[11px] text-gray-400 dark:text-slate-500 mb-2">{subtitle}</p>}
...
export default function Distribution({ service, since }: { service: string; since?: string }) {
  ...
  if (dist.n === 0) return <p className="text-sm text-gray-400 dark:text-slate-500">No traces in this window.</p>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      ...
      <Card title="Suspected failure signals" subtitle="Label-free proxy signals (a trace may carry several)">
        {Object.keys(dist.by_failure_label).length ? (
          ...
          <p className="text-xs text-gray-400 dark:text-slate-500">No suspected failures in this window.</p>
        )}
      </Card>
      <Card title="Latency percentiles" subtitle="Total latency (ms)">
        <div className="grid grid-cols-4 gap-2 text-center">
          {(['p50', 'p90', 'p95', 'p99'] as const).map((p) => (
            <div key={p}>
              <p className="text-[10px] text-gray-400 dark:text-slate-500 uppercase">{p}</p>
              <p className="text-lg font-bold text-gray-800 dark:text-slate-100 tabular-nums">{lat[p]?.toFixed(0)}</p>
            </div>
          ))}
        </div>
      </Card>
