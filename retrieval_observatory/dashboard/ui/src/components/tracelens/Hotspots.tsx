...
export default function Hotspots({ service, since }: { service: string; since?: string }) {
  ...
  if (!hotspots) return <div className="text-sm text-gray-400">Finding hotspots…</div>
  if (hotspots.length === 0) return <p className="text-sm text-gray-400">No suspected-failure segments in this window.</p>
  ...
  return (
    <div>
        ...
        {hotspots.map((h, i) => (
          <div key={i} className="rounded-lg border border-gray-200 bg-white p-3 flex items-center justify-between">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-xs font-mono text-gray-600">{h.pipeline}</span>
            </div>
      ...
      <div className="mt-4 rounded-lg border border-dashed border-amber-300 bg-amber-50/60 p-3 text-xs text-amber-800">
        ...
        <code className="mx-1 px-1 rounded bg-white border border-amber-200">retobs forge run --corpus your_corpus.jsonl --scenario-types temporal,alias</code>
