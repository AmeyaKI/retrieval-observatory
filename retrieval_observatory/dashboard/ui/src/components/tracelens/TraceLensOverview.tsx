...
function Kpi({ label, value, hint, tone }: { label: string; value: string; hint: string; tone?: 'bad' | 'warn' | 'ok' }) {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4" title={hint}>
  ...
  if (!summary) {
    return (
      <div className="flex items-center gap-2 text-gray-400 text-sm">
        <div className="animate-spin rounded-full h-4 w-4 border-2 border-gray-300 border-t-teal-500" />
        Loading summary…
      </div>
    )
  }
