...
export default function Clusters({ service, since }: { service: string; since?: string }) {
  ...
  if (clusters.length === 0) return <p className="text-sm text-gray-400 dark:text-slate-500">No traffic to cluster in this window.</p>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {clusters.map((c) => (
          <div key={c.cluster} className="rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-3">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-800 dark:text-slate-100">{c.cluster}</span>
              <span className="text-xs text-gray-400 dark:text-slate-500">{c.size} ({Math.round(c.share * 100)}%)</span>
            </div>
            <div className="flex gap-4 mt-1.5 text-[11px] text-gray-500 dark:text-slate-400">
            ...
            {c.examples.length > 0 && (
              <ul className="mt-2 space-y-0.5">
                {c.examples.map((ex, i) => (
                  <li key={i} className="text-[11px] text-gray-500 dark:text-slate-400 truncate">· {ex}</li>
                ))}
              </ul>
            )}
          </div>
