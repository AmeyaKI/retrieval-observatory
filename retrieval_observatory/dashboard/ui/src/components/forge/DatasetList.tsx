          ...
          <li key={d.dataset_id}>
            <button
              ...
              className={`w-full text-left rounded-lg border p-3 transition-colors ${
                active ? 'border-amber-300 bg-amber-50' : 'border-gray-200 bg-white hover:border-gray-300'
              }`}
            >
              <p className="text-sm font-semibold text-gray-900 truncate font-mono">{d.dataset_id}</p>
              <p className="text-xs text-gray-400 truncate mt-0.5">{corpusName} · {formatDate(d.created_at)}</p>
              <p className="text-xs text-gray-500 mt-1.5">
                <span className="font-medium text-gray-700">{s.total_scenarios ?? 0}</span> scenarios
                {' · '}
                <span className="font-medium text-gray-700">{s.total_queries ?? 0}</span> queries
              </p>
