...
/** Primary modes first; Compare is secondary (multi-run is opt-in). */
const PRIMARY_MODES: Array<{ id: Mode; label: string; icon: string }> = [
...
const SECONDARY_MODES: Array<{ id: Mode; label: string; icon: string }> = [
...
export default function ModeRail({ mode, onSelect, onOpenTour, showTourLink }: Props) {
  ...
  const renderMode = (item: { id: Mode; label: string; icon: string }, muted = false) => {
  ...
  return (
    <nav aria-label="Primary" className="fixed sm:static bottom-0 inset-x-0 z-40 h-16 sm:h-auto sm:w-24 bg-surface border-t sm:border-t-0 sm:hairline-r border-slate-200 dark:border-slate-700 flex sm:flex-col items-center px-1 sm:px-0 sm:py-3 gap-0 sm:gap-1">
      ...
      {PRIMARY_MODES.map((item) => renderMode(item))}
      <div className="hidden sm:block w-full px-2 my-1">
        <div className="h-px bg-slate-200 dark:bg-slate-700" />
        <p className="mt-1 text-[9px] uppercase tracking-wide text-ink-faint text-center">More</p>
      </div>
      {SECONDARY_MODES.map((item) => renderMode(item, true))}
