...
function outcomeHint(item: QueryLabelRow): string {
  const bucket = item.actual_bucket || 'unknown'
  if (item.agreement === 'mismatch') return `${bucket} · prediction mismatch`
  return bucket
}
  ...
  return (
    <div className="border border-gray-200 dark:border-slate-700 rounded bg-white dark:bg-slate-900">
      <p className="text-xs text-gray-500 dark:text-slate-400 px-3 pt-3 pb-1">
        Open a query to see expected vs retrieved chunks, a stage flowchart, and TP/FP/FN/TN attribution.
      ...
      <div className="p-3 border-b border-gray-100 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3">
        <input
          ...
          placeholder="Filter by query text or label"
        />
      ...
      <div className="overflow-x-auto max-h-[28rem]">
        <table className="min-w-full text-xs">
          <thead className="bg-gray-50 dark:bg-slate-800/60 sticky top-0">
            <tr>
              <th className="text-left px-3 py-2 w-[45%]">Query</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
            {visible.map((item) => (
              <tr key={item.query_id} className="hover:bg-gray-50 dark:hover:bg-slate-800/40">
                <td className="px-3 py-2.5">
                  <a
                    href={`#/runs/${encodeURIComponent(runId)}/queries/${encodeURIComponent(item.query_id)}`}
                    className="block text-sm text-ink hover:text-indigo-700 leading-snug"
                    title="Open query diagnosis"
                  >
                    {item.query_text?.trim() || (
                      <span className="font-mono text-ink-muted">{item.query_id}</span>
                    )}
                  </a>
                  {(item.predicted_risks?.length ?? 0) > 0 && (
                    <p className="mt-1 text-[10px] text-amber-700 dark:text-amber-300">
                      risks: {item.predicted_risks!.join(', ')}
                    </p>
                  )}
                </td>
                <td className="px-3 py-2 capitalize text-ink-muted">{outcomeHint(item)}</td>
                <td className="px-3 py-2">
                  <span className="block">{item.predicted_difficulty || '—'}</span>
                ...
                <td className="px-3 py-2">
                  <span className="font-mono text-[10px] text-ink-faint" title={item.query_id}>
                    {item.query_id.length > 14 ? `${item.query_id.slice(0, 12)}…` : item.query_id}
                  </span>
                  <a
                    href={`#/queries/${encodeURIComponent(item.query_id)}`}
                    className="ml-1.5 text-[10px] text-gray-400 hover:text-indigo-600"
                    title="Cross-run lineage"
                  >
                    lineage
                  </a>
                </td>
