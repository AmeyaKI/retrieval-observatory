...
def test_llm_judge_labels_used(tmp_path, monkeypatch):
    ...
    rep = ro.evaluate(_retrieve, queries=queries, corpus={"d1": "x", "d2": "y"}, k=5,
...
def test_generate_testset_from_corpus(tmp_path):
    """Test Sets synthesizes queries + qrels from a corpus with detectable scenarios (no API key)."""
    corpus = {
    ...
    if queries:  # rule-based generation is scenario-dependent
        rep = ro.evaluate(lambda q: list(ds.corpus.keys()), dataset=ds, k=5,
